...
public class MealsArray {
    private ArrayList<MealsArray> meals = new ArrayList<MealsArray>();
    private int i = 0;
    ...
    public ArrayList<MealsArray> getMeals() {
        return meals;
    }

    public void printAllMeals() {
        for (MealsArray item: meals) {
            if (item != null) {
    ...
    public void printByNameSearch(String s) {
        for (MealsArray item: meals) {
            if (item != null && item.getMeals().indexOf(s) >= 0) {
                System.out.println(item);
            }
